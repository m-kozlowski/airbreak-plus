...
void init_tracking(tracking_t *tr)
{
  ...
  tr->recent.te = 2.0f;
  tr->recent.ti = 1.6f;
  tr->recent.volume_max = 0.3f;
  tr->recent.exh_maxflow = -25.0f;
  tr->recent.inh_maxflow = 25.0f; // Init with reasonable-ish defaults in case code wants to rely on these
  init_breath(&tr->last);
...
void update_tracking(tracking_t *tr)
{
  ...
  if (tr->st_inhaling)
  {
    ...
    {
      tr->st_pre_cycle += 1;
    } // + (*flow_compensated<0.0f)
    else
    {
      tr->st_pre_cycle = max(tr->st_pre_cycle - 1, 0);
    } // Don't just reset it
  }
  else
  {
    ...
    if (flow2 > sens_trigger)
    {
      tr->st_pre_trigger += 1;
    } // Slightly higher than baseline trigger sens
    else
    {
      tr->st_pre_trigger = 0;
    } // This has to reset, flow doesn't slow down during the start of a breath
  }
